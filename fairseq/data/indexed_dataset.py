...
def make_dataset(path, impl, fix_lua_indexing=False, dictionary=None, source=True, sizes=None, motif_list=None,
    ...
    if impl == "raw" and IndexedRawTextDataset.exists(path):
        ...
        return IndexedRawTextDataset(path, dictionary, source=source, split=split, protein=protein)
    elif impl == "coor" and CoordinateDataset.exists(path):
        return CoordinateDataset(path, motif_list, split=split, protein=protein)
    elif impl == "motif" and ProteinMotifDataset.exists(path):
        return ProteinMotifDataset(path, sizes, epoch, train, split=split, protein=protein)
    elif impl == "pdb" and ProteinPDBDataset.exists(path):
        return ProteinPDBDataset(path, split=split, protein=protein)
    elif impl == "ec" and ECTreeDataset.exists(path):
        return ECTreeDataset(path, split=split, protein=protein)
    elif impl == "sub_atom" and SubstrateAtomDataset.exists(path):
        return SubstrateAtomDataset(path, split=split, protein=protein)
    elif impl == "sub_coor" and SubstrateCoordinateDataset.exists(path):
        return SubstrateCoordinateDataset(path, split=split, protein=protein)
    elif impl == "binding" and SubstrateBindingDataset.exists(path):
        return SubstrateBindingDataset(path, split=split, protein=protein)
...
def dataset_exists(path, impl):
    if impl == "raw":
        ...
        return IndexedBinarizedDataset.exists(path)
    else:
...
class IndexedRawTextDataset(FairseqDataset):
    ...
    def __init__(self, path, dictionary, source=True, append_eos=True, reverse_order=False, split="train", protein=None):
        self.tokens_list = []
        self.lines = []
        self.sizes = []
        self.source = source
...
class CoordinateDataset(FairseqDataset):
    ...
    def __init__(self, path, motif_list, split="train", protein=None):
        self.coors_list = []
        self.lines = []
        self.sizes = []
        self.centers = []
        self.read_data(path, motif_list, split, protein)
        self.size = len(self.coors_list)

    def read_data(self, path, motif_list, split, protein):
        f = open(path)
        data = json.load(f)
...
class ProteinMotifDataset(FairseqDataset):
    ...
    def __init__(self, path, dataset_sizes, epoch, train, split="train", protein=None):
        self.motif_list = []
        self.sizes = []
        self.epoch = epoch
        self.read_data(path, dataset_sizes, self.epoch, train, split, protein)
        self.size = len(self.motif_list)

    def read_data(self, path, dataset_sizes, epoch, train, split, protein):
        f = open(path)
        data = json.load(f)
...
class NCBITaxonomyDataset(FairseqDataset):
    ...
    def __init__(self, path, split="train", protein=None):
        ...
        self.ncbi2id = json.load(open(
            "/mnt/gemini/data2/zhenqiaosong/protein_design/data/ncbi2id.json", "r"))
        self.sizes = []
        self.read_data(path, split, protein)
    ...
    def read_data(self, path, split, protein):
        f = open(path)
        data = json.load(f)
        # ec4_train = data[protein]["train"]["ec4"]
        # ec4 = data[protein][split]["ec4"]
...
class ECTreeDataset(FairseqDataset):
    """
    Takes a text file as input and binarizes it in memory at instantiation.
    Original lines are also kept in memory"""
    ...
    def __init__(self, path, split="train", protein=None):
        ...
        self.ec3_list = []
        self.ec4_list = []
        self.ec_dict = json.load(open(
            "data/ec_four_level_id_dict.json",
            "r"))
        self.ec12id = self.ec_dict["level1"]
        self.ec22id = self.ec_dict["level2"]
        self.ec32id = self.ec_dict["level3"]
        self.ec42id = self.ec_dict["level4"]
        self.sizes = []
        self.read_data(path, split, protein)
        self.size = len(self.ec3_list)
    ...
    def read_data(self, path, split, protein):
        ...
        if split == "train":
            for protein in data:
                this_ec4 = data[protein][split]["ec4"]
                this_ec3 = [protein for _ in range(len(this_ec4))]
                this_ec2 = [".".join(protein.split(".")[: 2]) for _ in range(len(this_ec4))]
                this_ec1 = [protein.split(".")[0] for _ in range(len(this_ec4))]
                ec3.extend(this_ec3)
                ec4.extend(this_ec4)
                ec2.extend(this_ec2)
                ec1.extend(this_ec1)
            ...
            #     if ec4[i] not in ec4_train:
            #         ec4[i] = random.choice(ec4_train)
            ec4 = data[protein][split]["ec4"]
            ec3 = [protein for _ in range(len(ec4))]
            ec2 = [".".join(protein.split(".")[: 2]) for _ in range(len(ec4))]
            ec1 = [protein.split(".")[0] for _ in range(len(ec4))]

        for ec1_item, ec2_item, ec3_item, ec4_item in zip(ec1, ec2, ec3, ec4):
            self.ec1_list.append(self.ec12id[ec1_item])
            self.ec2_list.append(self.ec22id[ec2_item])
            self.ec3_list.append(self.ec32id[ec3_item])
            self.ec4_list.append(self.ec42id[ec4_item])
            self.sizes.append(1)
        self.sizes = np.array(self.sizes)
    ...
    def __getitem__(self, i):
        self.check_index(i)
        return (self.ec1_list[i], self.ec2_list[i], self.ec3_list[i], self.ec4_list[i])
...
class SubstrateAtomDataset(FairseqDataset):
    """Takes a text file as input and binarizes it in memory at instantiation.
    Original lines are also kept in memory"""
    ...
    def read_data(self, path, split, protein):
        ...
        for protein in data:
            if split == "train":
                lines.extend(data[protein][split]["substrate_feat"])
            else:
                if split in data[protein]:
                    lines.extend(data[protein][split]["substrate_feat"])
...
class SubstrateCoordinateDataset(FairseqDataset):
    """
    Takes a text file as input and binarizes it in memory at instantiation.
    Original lines are also kept in memory"""
    ...
    def read_data(self, path, split, protein):
        ...
        for protein in data:
            if split == "train":
                lines.extend(data[protein][split]["substrate_coor"])
            else:
                if split in data[protein]:
                    lines.extend(data[protein][split]["substrate_coor"])
...
class LigandAtomDataset(FairseqDataset):
    ...
    def read_data(self, path, split, protein):
        ...
        # lines = data[protein][split]["substrate_feat"]  # [num, L, 5]
        lines = []
        for protein in data:
            if split == "train":
...
class LigandCoordinateDataset(FairseqDataset):
    ...
    def read_data(self, path, split, protein):
        f = open(path)
        data = json.load(f)
        # lines = data[protein][split]["substrate_coor"]
        lines = []
        for protein in data:
            if split == "train":
...
class LigandBindingDataset(FairseqDataset):
    ...
    def read_data(self, path, split, protein):
        f = open(path)
        data = json.load(f)
        # lines = data[protein][split]["binding"]
...
class SubstrateBindingDataset(FairseqDataset):
    """Takes a text file as input and binarizes it in memory at instantiation.
    Original lines are also kept in memory"""
    ...
    def read_data(self, path, split, protein):
        ...
        for protein in data:
            if split == "train":
                lines.extend(data[protein][split]["binding"])
            else:
                if split in data[protein]:
                    lines.extend(data[protein][split]["binding"])
